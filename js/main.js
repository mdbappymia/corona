...
axios
  ...
  .then(({ data }) => {
    ...
    function allCountrySet(c) {
      ...
      menutc.appendChild(createMenuItem(c.TotalConfirmed)).className =
        "c-border";
      ...
      menutr.appendChild(createMenuItem(c.TotalRecovered)).className =
        "c-border";

      const menu = document.querySelector("#container");
      menu.appendChild(createMenuItem("Country Name: " + c.Country)).className =
        "allCountryName";
      menu.appendChild(
        createMenuItem("New Confirmed: " + c.NewConfirmed)
      ).className = "allCountryTTCase bg-warning";
      menu.appendChild(createMenuItem("New Deaths: " + c.NewDeaths)).className =
        "allCountryTTDeaths bg-danger";
      menu.appendChild(
        createMenuItem("New Recovered: " + c.NewRecovered)
      ).className = `allCountryTTRecovered bg-success`;
      menu.appendChild(
        createMenuItem("Total Confirmed: " + c.TotalConfirmed)
      ).className = "allCountryTTCase";
      menu.appendChild(
        createMenuItem("Total Deaths: " + c.TotalDeaths)
      ).className = "allCountryTDeadts";
      menu.appendChild(
        createMenuItem("Total Recovered: " + c.TotalRecovered)
      ).className = "allCountryTRecovered";
    }

    inputCountry.addEventListener("keypress", function (e) {
      if (e.key === "Enter") {
        if (e.target.value != "") {
          var resultObject = search(e.target.value.trim().toUpperCase(), arr);
